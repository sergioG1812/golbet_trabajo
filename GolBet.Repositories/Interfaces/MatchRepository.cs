...
public class MatchRepository : GenericRepository<Match>, IMatchRepository
{
    ...
    {
        var query = _dbSet.Include(m => m.HomeTeam).Include(m => m.AwayTeam)
            .Where(m => m.IsActive).AsNoTracking().AsQueryable();
    ...
    public async Task<Match?> GetByIdWithDetailsAsync(int id) =>
        await _dbSet.Include(m => m.HomeTeam).Include(m => m.AwayTeam).Include(m => m.Bets)
